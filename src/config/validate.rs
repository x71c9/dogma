...
pub fn validate(config: &DogmaConfig) -> Result<()> {
    ...
    let secret_groups: Vec<&str> = config.secrets.keys().map(String::as_str).collect();
        ...
        if let super::HostnameField::PerEnv(map) = &machine.hostname {
            for env in &config.env {
                if !map.contains_key(env) {
                    errors.push(format!(
                        "machines.{host}.hostname: missing env '{env}'"
                    ));
                }
            }
        }
        ...
        for (key, leaf) in fields {
            ...
            match leaf {
                ...
                SecretLeaf::FromInfra { unit, output, .. } => {
                    if output.is_empty() {
                        errors.push(format!("{path}: from:infra requires an 'output' field"));
                    }
                    if unit.is_empty() {
    ...
    if !errors.is_empty() {
        let msg = errors.join("\n");
        bail!("{}\n\n{} error(s) found — fix dogma.yml and re-run", msg, errors.len());
    }
