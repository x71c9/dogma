...
pub fn read_cached(
    ...
    cache[unit][output]
        ...
        .map(str::to_string)
        .ok_or_else(|| {
            anyhow::anyhow!("output '{output}' not found in unit '{unit}' for env '{env}'")
        })
}
...
fn discover_units(infra_dir: &Path) -> Result<Vec<String>> {
    ...
    for entry in std::fs::read_dir(infra_dir)
        .with_context(|| format!("cannot read infra dir: {}", infra_dir.display()))?
    {
        let entry = entry?;
        let path = entry.path();
        if !path.is_dir() {
