...
pub fn run(config: &DogmaConfig, repo_root: &Path, _env: &str, refetch: bool) -> Result<()> {
    ...
    for (host_name, machine) in &config.machines {
        for env_name in &all_envs {
            ...
            let ip = match resolve_ip(config, repo_root, host_name, env_name) {
                Ok(ip) => ip,
                Err(e) => {
                    log_warn!("sops: {host_name}/{env_name}: cannot resolve IP — skipping ({e})");
                    continue;
                }
            };
...
fn collect_admin_keys(config: &DogmaConfig) -> Result<(Vec<String>, Vec<String>)> {
    let mut pgp = Vec::new();
    let mut age = Vec::new();
...
fn fetch_age_key(ip: &str, hostname: &str, cache_file: &Path) -> Result<String> {
    ...
    let ssh_pubkey = stdout
        ...
        .ok_or_else(|| anyhow::anyhow!("{hostname}: ssh-keyscan got no ed25519 key from {ip}"))?;
    ...
    use std::io::Write;
    child.stdin.take().unwrap().write_all(ssh_pubkey.as_bytes())?;
    ...
    std::fs::write(cache_file, format!("{age_key}\n"))
        .with_context(|| format!("failed to cache age key: {}", cache_file.display()))?;
